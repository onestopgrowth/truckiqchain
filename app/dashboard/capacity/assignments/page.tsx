...
async function fetchAssignments() {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SITE_URL || ""}/api/assignments?role=carrier`,
...
export default async function CarrierAssignmentsPage() {
  ...
  const { data } = await sb
    .from("assignments")
    .select(
      "id,load_id,status,requested_at,accepted_at,booked_at,in_transit_at,delivered_at,completed_at,pod_file_path"
    )
...
function ActionButtons({ assignment }: { assignment: any }) {
  ...
  async function uploadPod(file: File) {
    ...
    const res = await fetch(`/api/assignments/${assignment.id}/pod`, {
      method: "POST",
      body: form,
    });
  ...
  if (status === "booked")
    allowed.push({ action: "start", label: "Start Transit" });
  if (status === "in_transit")
    allowed.push({ action: "deliver", label: "Mark Delivered" });
  if (status === "delivered")
    allowed.push({ action: "complete", label: "Complete" });
  if (["requested", "accepted", "booked"].includes(status))
    allowed.push({ action: "cancel", label: "Cancel" });
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await fetch(`/api/assignments/${assignment.id}/waypoints`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
