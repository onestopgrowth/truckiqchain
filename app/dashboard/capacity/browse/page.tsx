...
export default async function BrowseCapacityCallsPage({
  ...
  const { data: capacityCalls } = await query.order("created_at", {
    ascending: false,
  });
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Find loads that match your equipment and experience
          </p>
        </div>
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription>
                      Posted by{" "}
                      {call.profiles?.company_name || call.profiles?.email}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      {call.pickup_date
                        ? new Date(call.pickup_date).toLocaleDateString()
                        : "TBD"}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm">
                      {call.delivery_date
                        ? new Date(call.delivery_date).toLocaleDateString()
                        : "TBD"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-4 pt-4 border-t">
                  <div className="flex gap-4 text-sm">
                    {call.weight && (
                      <span>Weight: {call.weight.toLocaleString()} lbs</span>
                    )}
                    {call.total_rate && (
                      <span className="font-semibold">
                        Rate: ${call.total_rate.toLocaleString()}
                      </span>
                    )}
                    {call.rate_per_mile && (
                      <span>Per Mile: ${call.rate_per_mile}</span>
                    )}
                  </div>
