...
import { notifyAssignmentTransition } from "@/lib/notifications";
...
export async function PATCH(
  ...
  try {
    ...
    function set(ns: string, tf: string) {
      nextStatus = ns;
      timestampField = tf;
    }
    ...
    switch (action) {
      ...
      case "cancel":
        if (
          (isCarrier &&
            ["requested", "accepted", "booked"].includes(current)) ||
          (isOwner && ["requested", "accepted", "booked"].includes(current))
        ) {
          set("cancelled", "cancelled_at");
        }
        break;
      case "start":
        if (isCarrier && current === "booked") {
          set("in_transit", "in_transit_at");
        }
        break;
      case "deliver":
        if (isCarrier && current === "in_transit") {
          set("delivered", "delivered_at");
        }
        break;
      case "complete":
        if (isOwner && current === "delivered") {
    ...
    if (action === "complete" && pod_url) update.pod_url = pod_url;
