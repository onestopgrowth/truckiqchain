...
export async function POST(
  ...
  try {
    ...
    if (assignment.carrier_user_id !== user.id)
      return NextResponse.json({ error: "forbidden" }, { status: 403 });
    if (assignment.status !== "in_transit" && assignment.status !== "delivered")
    ...
    const { error: upErr } = await sb
      ...
      .update({
        pod_file_path: path,
        pod_mime_type: file.type || null,
        pod_uploaded_at: new Date().toISOString(),
        status:
          assignment.status === "in_transit" ? "delivered" : assignment.status,
        delivered_at:
