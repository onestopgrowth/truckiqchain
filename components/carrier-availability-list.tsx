...
export default function CarrierAvailabilityList({ items }: { items: any[] }) {
  ...
  async function remove(id:string){
    ...
    const res = await fetch(`/api/carrier/availability/${id}`, { method: 'DELETE' });
    if(res.ok) setRows(r=>r.filter(x=>x.id!==id));
  }
  function refreshed(){ window.location.reload(); }
  return (
    <div className='space-y-3'>
      {rows.map(r => (
        <div key={r.id} className='border rounded p-3 text-xs flex flex-col gap-2'>
          <div className='flex justify-between'>
            <div className='font-mono text-[11px]'>{r.start_at?.slice(0,16)} → {r.end_at?.slice(0,16)}</div>
          ...
          <div className='flex flex-wrap gap-1'>
            {r.equipment?.map((e:string)=> <Badge key={e} variant='secondary' className='text-[10px]'>{e}</Badge>)}
          </div>
        </div>
      ))}
      {!rows.length && <p className='text-xs text-muted-foreground'>No availability windows yet.</p>}
      {editing && (
